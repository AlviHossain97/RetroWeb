...
// bn_sound_items.h is auto-generated by Butano from gba_project/audio/*.wav.
// Gated behind ENABLE_AUDIO (defined in core/config.h, undefined by default
// for the silent-corpus build) — with the audio dir empty, Butano won't
// emit this header so referencing it would fail to compile.
#ifdef ENABLE_AUDIO
...
#else
        (void)id;  // silent build — audio infrastructure preserved as engineering work, see core/config.h
#endif
    }
...
#ifdef ENABLE_AUDIO
    // The sfx_item / bgm_item mappers reference bn::sound_items::* symbols
    // that Butano auto-generates from gba_project/audio/*.wav. They only
    // compile when audio is enabled; the wav directory is currently empty
    // (see gba_project/audio/README.md).
