...
| [`Jellyfin.sh`](Jellyfin.sh) | `/home/pi/RetroPie/roms/jellyfin/Jellyfin.sh` | Loop 2 launcher. Plays the splash, then invokes `/usr/bin/kodi-standalone` (the distro's official session-manager wrapper) which owns the VT and framebuffer lifecycle natively. **Rewritten 2026-04-25** after an earlier xinit-based clean-exit attempt locked the Pi on Kodi exit; the new approach drops the xinit + openbox + manual VT-switching layer entirely. Persistent log at `/home/pi/pistation-jellyfin.log`. The pre-2026-04-25 original is preserved on the Pi as `Jellyfin.sh.bak-orig-*`. |
...
| Loop 2 launcher (Jellyfin → Kodi) | Rewritten to delegate to `/usr/bin/kodi-standalone`; persistent log at `/home/pi/pistation-jellyfin.log`. **Needs user testing** to confirm Kodi exits cleanly back to EmulationStation. |
